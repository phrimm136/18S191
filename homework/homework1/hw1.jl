...
function mean_colors(image)
	return sum(image) / size(image, 1) / size(image, 2)
end
...
The image is unrecognisable with intensity more than one because every RGB has a possibility to  become totally different one when the intensity is more than one.
"""
...
# ╔═╡ f0e8d678-02dd-11eb-0d41-4d658603f467
colored_line(blur_1D(v, 5))

# ╔═╡ ca1ac5f4-ee1c-11ea-3d00-ff5268866f87
colored_line(blur_1D(v, 10))
...
function gaussian_kernel(n)
	l = (n-1)/2
	gaus = [exp(-x^2/2)/√2π for x in -l:l]
...
function extend_mat(M::AbstractMatrix, i, j)
	return M[min(size(M, 1), max(1, i)), min(size(M, 2), max(1, j))]
end
...
# ╔═╡ 697a04f0-02df-11eb-0d43-85137b1cccef
...
function with_gaussian_blur(image)
	gaus = gaussian_kernel_2D(3)
	return convolve_image(image, gaus)
end

# ╔═╡ 8ae59674-ee18-11ea-3815-f50713d0fa08
md"_Let's make it interactive. 💫_"

# ╔═╡ f2cf53c8-02df-11eb-0757-21f882424f10
...
function with_sobel_edge_detect(image)
	G_x = convolve_image(image, [1 0 -1; 2 0 -2; 1 0 -1])
	G_y = convolve_image(image, [1 2 1; 0 0 0; -1 -2 -1])
	
	return Gray.(sqrt.(Gray.(G_x) .^ 2 + Gray.(G_y) .^ 2))
end

# ╔═╡ 33da925c-02e1-11eb-32c0-099d3e6ab602
